...
export function additionalConflict(edge, byId) {
  const a = byId.get(edge.sourceAssetId ?? edge.sourceNodeId), b = byId.get(edge.targetAssetId ?? edge.targetNodeId)
...
export function correctAdditionalMounts(relations, assets) {
  ...
  return [...relations.filter(r => !overrides.has(r.sourceAssetId) && !excluded.has(r.sourceAssetId)),
